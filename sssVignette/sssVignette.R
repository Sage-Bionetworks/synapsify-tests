...
trainSet <- t(exEstrogen[ , 1:4]) # Note that the matrix is transposed
validSet <- t(exEstrogen[ , 5:8])
trainPheno <- ifelse(exDesign$ES[1:4] == "P", 1, 0)
validPheno <- ifelse(exDesign$ES[5:8] == "p", 1, 0)

# Fit the model
sssFit <- sss(trainPheno ~ trainSet)
